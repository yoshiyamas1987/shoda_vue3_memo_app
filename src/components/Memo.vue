<template>
  <section class="alert alert-primary">
    ...
    <div>
      <button class="btn btn-info m-2" @click="insert">
        save
      </button>
      <transition name="del">
        <button
...
export default {
  setup(props) {
    ...
    // フラグの初期化
    const set_flg = () => {
      if ( data.find_flg || data.sel_flg != false ){
        data.find_flg = false
        data.sel_flg = false
        data.title = ''
        data.content = ''
    ...
    const page = computed({
      ...
      set: (p) => {
        var pg = p > (data.store.state.memo.length - 1) / data.num_per_page ?
